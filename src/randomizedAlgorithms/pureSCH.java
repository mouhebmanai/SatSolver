...
public class pureSCH extends RandomizedAlgorithm implements  CNFSATSolver{
     ...
     *  the error probability is less than e^-m
     */
     public void setExpErrorProbability (CnfFormula formula , double m, int k) {
        this.Repetitions = (int) Math.ceil( m *  3.0/2 * Math.pow(2.0*(k-1)/k , formula.NumberOfVariables()) );
     }
